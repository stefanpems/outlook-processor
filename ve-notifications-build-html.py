"""
Build an HTML report from Viva Engage notification digest summaries.
Uses the same template as engage_build_html.py.

Usage:
  python ve-notifications-build-html.py --input summaries.json
...
TYPE_LABELS = {
    ...
    "praise": ("Praise", "#f39c12"),
    "poll": ("Poll", "#3498db"),
    "article": ("Article", "#1abc9c"),
}
...
def build_html(data):
    ...
    if date_from and date_to:
        ...
        title_text = f"PescoPedia Viva Engage Digest - From: {date_from_fmt} To: {date_to_fmt}"
        subtitle = f"From: {date_from_fmt} To: {date_to_fmt}"
    else:
        title_text = "PescoPedia Viva Engage Digest"
        subtitle = ""
    ...
    # Header
    h.append('<h1>PescoPedia Viva Engage Digest</h1>')
    if subtitle:
    ...
    for comm in communities:
        ...
        if not convs:
            ...
            for i, conv in enumerate(convs, 1):
                ...
                label, type_color = TYPE_LABELS.get(conv_type.lower(),
                                                     ("Discussion", "#3498db"))
    ...
    h.append('<div class="footer-bar">Generated by Viva Engage Notification Digest Pipeline</div>')
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build Viva Engage notification digest HTML.")
...
def main():
    ...
    if date_from and date_to:
        ...
        from_dot = datetime.now().strftime("%Y.%m.%d")
        to_dot = from_dot
