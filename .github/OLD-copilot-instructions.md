...
| File | Purpose |
|------|---------|
| `run_all.py` | Master orchestrator: email retrieval, enrichment, blog fetch, per-period HTML+XLSX reports. Exports `get_topic_color()`, `TOPIC_COLORS`, `COLOR_PALETTE`. |
| `step1_retrieve_emails.py` | Standalone email retrieval from Outlook Web via Playwright CDP. |
| `cache_blogs.py` | Fetches blog article text from URLs in `output/emails_*.json` and caches in `blog_cache/`. |
| `batch_all.py` | Batch SP item creation + Outlook email categorize/move. Safe search with `select_blog_emails()`. |
| `sp_create_items.py` | Single-item SP creation + email processing (interactive mode). |
| `fetch_blogposts.py` | Fetches all existing BlogPosts from SP list → `sp_blogposts.json`. |
| `fetch_sp_list.py` | Fetches "Ref Technologies New" list from SP → `tech_list.json`. |
| `update_sp_summaries.py` | Bulk-updates Summary field on existing SP items via REST MERGE. |
| `update_status_regen.py` | Syncs status from `batch_errors.json` → `deduped_matched.json`, regenerates consolidated reports. |
