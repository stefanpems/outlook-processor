...
| `yt_transcripts/` | Directory of downloaded YouTube video transcripts (`yt_<VIDEO_ID>.txt`). Populated by `yt_transcript.py`. Path configurable via `config.json → transcripts.yt_transcripts_dir`. |
| `teams_transcripts/` | Directory of downloaded Teams meeting recording transcripts (`<SHA256>.txt`). Populated by `pipeline_fetch_teams_meeting.py`. Path configurable via `config.json → transcripts.teams_transcripts_dir`. |
