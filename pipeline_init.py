"""
Initialize a blog/video notification processing session.
Creates XLSX + HTML report template files with date-range naming.

Usage: python pipeline_init.py --type blog --from-date YYYY-MM-DD --to-date YYYY-MM-DD
       python pipeline_init.py --type video --from-date YYYY-MM-DD --to-date YYYY-MM-DD
...
parser.add_argument("--type", choices=["blog", "video"], default="blog")
parser.add_argument("--from-date", required=True, help="Start date YYYY-MM-DD")
parser.add_argument("--to-date", required=True, help="End date YYYY-MM-DD")
args = parser.parse_args()

type_prefix = "Blog_Notifications" if args.type == "blog" else "Video_Notifications"
date_from_dot = args.from_date.replace("-", ".")
date_to_dot = args.to_date.replace("-", ".")

# ── Determine file name (add -NN suffix if collision) ────────────────────
base_name = f"{type_prefix}-Digest-From-{date_from_dot}-To-{date_to_dot}"
...
if os.path.exists(xlsx_path) or os.path.exists(html_path):
    nn = 2
    while True:
        base_name_nn = f"{base_name}-{nn:02d}"
        xlsx_path = os.path.join(OUTPUT, f"{base_name_nn}.xlsx")
        html_path = os.path.join(OUTPUT, f"{base_name_nn}.html")
        if not os.path.exists(xlsx_path) and not os.path.exists(html_path):
            base_name = base_name_nn
            break
        nn += 1
...
<title>{type_label} Notifications &mdash; Processing Session {date_range}</title>
<style>
...
<h1>{type_label} Notifications &mdash; {date_range}</h1>
<div class="stats" id="stats"></div>
<nav class="toc" id="toc"><h2>Topics</h2><ul id="toc-list"></ul></nav>
<div id="content"></div>
<footer>Generated by {type_label} Notification Pipeline</footer>
</body></html>""".format(
    type_label="Blog" if args.type == "blog" else "Video",
    date_range=f"From {date_from_dot} To {date_to_dot}",
)
